...
class tplMrvbWidgets
{
	...
	public static function mrvb_feed($w)
	{
		...
		foreach ($feed->items as $item) {
			...
			$li = $formitem;
			if (strpos($formitem,'%date%') !== false) $li = str_replace('%date%', $date, $li);
			if (strpos($formitem,'%title%') !== false) $li = str_replace('%title%', $link ? '<a href="'.html::escapeHTML($url).'">'.$title.'</a>' : $title, $li);
			if (strpos($formitem,'%date+title%') !== false) $li = str_replace('%date+title%', $link ? '<a href="'.html::escapeHTML($url).'">'.$date.$separator.$title.'</a>' : $date.$separator.$title, $li);
			$res .= '<li>'.$li.'</li>'."\n";
			$i++;
			if ($i >= $limit) {
				break;
	...
	public static function mrvb_LastPosts($w)
	{
		...
		if (empty($typepost)) $typepost = 'post';
	...
	public static function mrvb_Search($w)
	{
		...
		$res = ($w->title ? $w->renderTitle(html::escapeHTML($w->title))."\n" : '');
		$res .=
		'<form action="'.dcCore::app()->blog->url.'" method="get">'."\n".
		'<fieldset>'."\n".
